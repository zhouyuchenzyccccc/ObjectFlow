...
def _compute_metrics(
    ...
    point_track_is_robot: np.ndarray = None,
    point_motion_object_group_id: np.ndarray = None,
    ...
    if point_track_is_robot is not None:
        if point_track_is_robot.shape[0] != n_points:
            raise ValueError(
                "point_track_is_robot shape mismatch: "
                f"expected ({n_points},), got {point_track_is_robot.shape}"
            )
        metrics["robot_point_ratio"] = float(np.mean(point_track_is_robot.astype(np.float32)))

    if point_motion_object_group_id is not None:
        if point_motion_object_group_id.shape[0] != n_points:
            raise ValueError(
                "point_motion_object_group_id shape mismatch: "
    ...
    if phase_label is not None:
        if phase_label.shape[0] != max(t_steps - 1, 0):
            raise ValueError(
                "phase_label shape mismatch: "
                f"expected ({max(t_steps - 1, 0)},), got {phase_label.shape}"
            )
        if phase_label.shape[0] <= 1:
            metrics["num_phase_transitions"] = 0.0
        else:
            metrics["num_phase_transitions"] = float(np.sum(phase_label[1:] != phase_label[:-1]))
...
def _plot_episode(
    ...
    sample_points: int,
    phase_label: np.ndarray = None,
    phase_robot_active_ratio: np.ndarray = None,
    phase_object_active_ratio: np.ndarray = None,
) -> None:
    """Create a diagnostic visualization for an episode."""
    t_steps, n_points, _ = point_abs.shape
    point_count = min(sample_points, n_points)
    idx = np.arange(point_count)

    has_phase = phase_label is not None and phase_label.shape[0] > 0
    cols = 4 if has_phase else 3
    fig = plt.figure(figsize=(5.0 * cols, 4.8))
    ...
    if has_phase:
        ...
        t_axis = np.arange(phase_label.shape[0])
        ax4.step(t_axis, phase_label, where="post", linewidth=1.5, label="phase_label")
        if phase_robot_active_ratio is not None and phase_robot_active_ratio.shape[0] == phase_label.shape[0]:
            ax4.plot(t_axis, phase_robot_active_ratio, linewidth=1.1, label="robot_active_ratio")
        if phase_object_active_ratio is not None and phase_object_active_ratio.shape[0] == phase_label.shape[0]:
            ax4.plot(t_axis, phase_object_active_ratio, linewidth=1.1, label="object_active_ratio")
        ax4.set_title("Temporal Phase Signals")
        ax4.set_xlabel("t")
    ...
    for hdf5_path in hdf5_files:
        with h5py.File(hdf5_path, "r") as f:
            ...
            for demo_key in sorted(f["data"].keys()):
                ...
                point_track_is_robot = obs["point_track_is_robot"][()] if "point_track_is_robot" in obs else None
                point_motion_object_group_id = (
                    obs["point_motion_object_group_id"][()] if "point_motion_object_group_id" in obs else None
                )
                phase_label = obs["phase_label"][()] if "phase_label" in obs else None
                phase_robot_active_ratio = (
                    obs["phase_robot_active_ratio"][()] if "phase_robot_active_ratio" in obs else None
                )
                phase_object_active_ratio = (
                    obs["phase_object_active_ratio"][()] if "phase_object_active_ratio" in obs else None
                )

                metrics = _compute_metrics(
                    point_abs,
                    point_disp,
                    point_track_is_robot=point_track_is_robot,
                    point_motion_object_group_id=point_motion_object_group_id,
                ...
                if checked_eps <= args.num_visualize:
                    ...
                    _plot_episode(
                        ...
                        phase_label=phase_label,
                        phase_robot_active_ratio=phase_robot_active_ratio,
                        phase_object_active_ratio=phase_object_active_ratio,
                    )
